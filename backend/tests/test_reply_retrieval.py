...
def test_reply_payload_uses_retrieved_subset(client, monkeypatch):
    ...
    monkeypatch.setattr(webhook, "select_relevant_products", fake_retrieval)
    monkeypatch.setattr(webhook, "AI_SERVICE_URL", "http://127.0.0.1:9")  # force fallback after retrieval
